...
export default function geoFilter(geoData: ? GeoData, activationRule : ActivationRule) {
    ...
    const ruleWithMaxSpecificity = matchingRules.reduce((currentMax, rule) => (!currentMax || getSpecificity(rule) > getSpecificity(currentMax) ? rule : currentMax), null);
