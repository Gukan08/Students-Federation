...
} from 'createScrollPositionChanges';
...
function processRules(activationRules, stateInputs, setRulesAction, logger, errorReporter): void {
    ...
    merge(
            ...
            createKeyboardEscape(), of (actions.setVisitorId({
                visitorId: getVisitorId(window.localStorage)
            })), of (setRulesAction), of (...activationRules).pipe(mergeMap(createTrigger(initialState))),
        )
        .pipe(
            scan((acc, action) => ({
                action,
...
export function preview({
    ...
    const action = actions.setActivationRulesPreview({
        ruleSrcPairs: upgradedPairs
    });
